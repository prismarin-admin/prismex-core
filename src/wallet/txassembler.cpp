    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional PRX outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes PRX and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::PRX_TO_PRX) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a PRX-to-PRX transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::PRX_ONLY;
        ...
        // If PRX-to-PRX fails, create a peg-out transaction
