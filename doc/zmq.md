...
The Prismex Core daemon can be configured to act as a trusted "border
router", implementing the prismex wire protocol and relay, making
...
The ZeroMQ feature in Prismex Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building prismexd:
...
For instance:
    ...
    $ prismexd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/prismexd.tx.raw \
...
These options can also be provided in prismex.conf.
...
From the perspective of prismexd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into prismexd directly. Furthermore, no information is
...
using. Prismexd appends an up-counting sequence number to each
